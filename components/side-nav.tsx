...
import { faCode, faContactBook, faDesktop, faHome, faUser, faUsers } from "@fortawesome/free-solid-svg-icons";
        ...
        <nav className="opacity-70 rounded-t-full rounded-b-full fixed top-24 right-5 nav-switch:top-24 nav-switch:right-20 p-3 flex flex-col items-center justify-center gap-y-7 bg-gray-800 border-[2px] border-gray-300">
            {navItems.map((item) => (
                <ScrollLink
                    ...
                    className={`${activeSection === item.to ? 'bg-white' : 'bg-gray-800'
                        } hover:text-prmry2 rounded-full w-8 h-8 flex items-center justify-center`}
                >
                    <FontAwesomeIcon
                        icon={item.icon}
                        className={`${activeSection === item.to ? 'text-gray-800' : 'text-gray-200'
                            } w-4 h-4`}
                    />
                </ScrollLink>
            ))}
